...
export class SellableCardsPageComponent implements OnInit {
  ...
  private _getExceededQuantity(card: CardModel): number {
    ...
    if (card.category === "LEADER") {
      return card.inventoryId ? (card.inventoryQuantity ?? 0) - leaderMinCardsAmount : 0;
    }
    if (card.category === "DON") {
      return card.inventoryId ? (card.inventoryQuantity ?? 0) - donMinCardsAmount : 0;
    }
    return card.inventoryId ? (card.inventoryQuantity ?? 0) - otherMinCardsAmount : 0;
  }
  ...
  private async _onAcceptSellCard(card: CardPreviewModel, quantity: number) {
    try {
      const inventory = (await this._inventoryService.upsertInventory(
        card.card,
        (card.card.inventoryQuantity ?? 0) - quantity
      ))[0]

      card.card.inventoryId = inventory.id;
      card.card.inventoryQuantity = inventory.quantity;
